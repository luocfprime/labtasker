...
async def periodic_task(db: AsyncDBService, interval_seconds: int):
...
async def lifespan(app: FastAPI):
    ...
    app.state.db = await AsyncDBService.init(config.mongodb_uri, config.db_name)
...
@app.get("/health")
async def health_check(db: AsyncDBService = Depends(get_db)):
...
async def create_queue(queue: QueueCreate, db: AsyncDBService = Depends(get_db)):
    """Create a new queue"""
...
async def get_queue(queue_id: str, db: AsyncDBService = Depends(get_db)):
...
async def delete_queue(
    ...
    db: AsyncDBService = Depends(get_db),
):
    """Delete a queue"""
