...
def main():
    ...
    #                   )
    parser.add_option(
        "-t", "--tree", default=False, action="store_true", help="Print AST tree"
    )
    parser.add_option(
        "-k",
        "--tokens",
    ...
    parser.add_option(
        "-s", "--sll", dest="sll", default=False, action="store_true", help="Show SLL"
    )
    parser.add_option(
        "-d",
        "--diagnostics",
    ...
    def process(input_stream, class_lexer, class_parser):
        ...
        if hasattr(parser, start_rule):
            ...
            print(
                "[ERROR] Can't find start rule '{}' in parser '{}'".format(
                    start_rule, parserName
                )
            )
