...
class RuleTransition(Transition):
    ...
    def __init__(
        ...
        self.ruleIndex = (
            ruleIndex  # ptr to the rule definition object for this rule ref
        )
        self.precedence = precedence
        self.followState = (
            followState  # what node to begin computations following ref to rule
        )
...
class PredicateTransition(AbstractPredicateTransition):
    ...
    def __init__(
        self, target: ATNState, ruleIndex: int, predIndex: int, isCtxDependent: bool
    ):
        super().__init__(target)
...
class NotSetTransition(SetTransition):
    ...
    def matches(self, symbol: int, minVocabSymbol: int, maxVocabSymbol: int):
        return (
            ...
            and not super(type(self), self).matches(
