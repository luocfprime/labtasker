...
class ATNConfig(object):
    ...
    def __init__(
        ...
        self.reachesIntoOuterContext = (
            0 if config is None else config.reachesIntoOuterContext
        )
        self.precedenceFilterSuppressed = (
            False if config is None else config.precedenceFilterSuppressed
        )
    ...
    def __eq__(self, other):
        if self is other:
            ...
            return (
                self.state.stateNumber == other.state.stateNumber
    ...
    def __hash__(self):
        return hash(
            (self.state.stateNumber, self.alt, self.context, self.semanticContext)
        )
    ...
    def __str__(self):
        with StringIO() as buf:
            ...
            if self.context is not None:
                ...
                buf.write("]")
            if (
                self.semanticContext is not None
                and self.semanticContext is not SemanticContext.NONE
            ):
                buf.write(",")
...
class LexerATNConfig(ATNConfig):
    ...
    def __init__(
        ...
        super().__init__(
            state=state, alt=alt, context=context, semantic=semantic, config=config
        )
        if config is not None:
        ...
        self.passedThroughNonGreedyDecision = (
            False if config is None else self.checkNonGreedyDecision(config, state)
        )
    ...
    def checkNonGreedyDecision(self, source: LexerATNConfig, target: ATNState):
        return (
            ...
            or isinstance(target, DecisionState)
            and target.nonGreedy
        )
