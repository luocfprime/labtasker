...
class LexerATNSimulator(ATNSimulator):
    ...
    def matchATN(self, input: InputStream):
        ...
        if LexerATNSimulator.debug:
            print(
                "DFA after matchATN: "
                + str(self.decisionToDFA[old_mode].toLexerString())
            )
    ...
    def failOrAccept(
        self, prevAccept: SimState, input: InputStream, reach: ATNConfigSet, t: int
    ):
    ...
    def getReachableConfigSet(
        self, input: InputStream, closure: ATNConfigSet, reach: ATNConfigSet, t: int
    ):
    ...
    def closure(
        ...
        if isinstance(config.state, RuleStopState):
            if LexerATNSimulator.debug:
                if self.recog is not None:
                    print(
                        "closure at",
                        self.recog.symbolicNames[config.state.ruleIndex],
                        "rule stop",
                        str(config),
                    )
                else:
            ...
            if config.context is not None and not config.context.isEmpty():
                for i in range(0, len(config.context)):
                    if (
                        ...
                        c = LexerATNConfig(
                            state=returnState, config=config, context=newContext
                        )
        ...
        if not config.state.epsilonOnlyTransitions:
            if (
                not currentAltReachedAcceptState
                or not config.passedThroughNonGreedyDecision
            ):
                configs.add(config)
    ...
    def evaluatePredicate(
        self, input: InputStream, ruleIndex: int, predIndex: int, speculative: bool
    ):
    ...
    def captureSimState(
        self, settings: SimState, input: InputStream, dfaState: DFAState
    ):
    ...
    def addDFAEdge(
        self, from_: DFAState, tk: int, to: DFAState = None, cfgs: ATNConfigSet = None
    ) -> DFAState:
    ...
    def addDFAState(self, configs: ATNConfigSet) -> DFAState:
        ...
        firstConfigWithRuleStopState = next(
            (cfg for cfg in configs if isinstance(cfg.state, RuleStopState)), None
        )
