...
class SingletonPredictionContext(PredictionContext):
    ...
    def __eq__(self, other):
        if self is other:
            ...
            return (
                self.returnState == other.returnState
                and self.parentCtx == other.parentCtx
            )
...
class ArrayPredictionContext(PredictionContext):
    ...
    def __eq__(self, other):
        if self is other:
            ...
            return (
                self.returnStates == other.returnStates
...
def merge(
    a: PredictionContext, b: PredictionContext, rootIsWildcard: bool, mergeCache: dict
):
    ...
    if isinstance(a, SingletonPredictionContext) and isinstance(
...
def mergeSingletons(
    ...
    if a.returnState == b.returnState:
        ...
        singleParent = None
        if a is b or (
            a.parentCtx is not None and a.parentCtx == b.parentCtx
        ):  # ax + bx = [a,b]x
...
# /
def mergeRoot(
    a: SingletonPredictionContext, b: SingletonPredictionContext, rootIsWildcard: bool
):
    if rootIsWildcard:
...
def mergeArrays(
    ...
    while i < len(a.returnStates) and j < len(b.returnStates):
        ...
        if a.returnStates[i] == b.returnStates[j]:
            ...
            # $+$ = $
            bothDollars = (
                payload == PredictionContext.EMPTY_RETURN_STATE
                and a_parent is None
                and b_parent is None
            )
            ax_ax = (
                a_parent is not None and b_parent is not None
            ) and a_parent == b_parent  # ax+ax -> ax
    ...
    if k < len(mergedParents):  # write index < last position; trim
        if k == 1:  # for just one merged element, return singleton top
            merged = SingletonPredictionContext.create(
...
def getCachedPredictionContext(
    context: PredictionContext, contextCache: PredictionContextCache, visited: dict
):
    if context.isEmpty():
        return context
...
def getAllContextNodes(
    context: PredictionContext, nodes: list = None, visited: dict = None
):
    if nodes is None:
        nodes = list()
