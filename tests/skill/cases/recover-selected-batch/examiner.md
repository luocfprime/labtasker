# recover-selected-batch — realistic workflow, version 1

Selected failed/cancelled work is pending and executable again, successful and unrelated experiments unchanged, no work executed. IDs may be preserved or deliberate resubmission explained only if no duplicate independently executable selected experiment remains; count complete outcome, not mandated lifecycle call.

Provision with `uv run python tests/skill/cases/recover-selected-batch/scripts/run.py setup`
