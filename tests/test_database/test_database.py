...
@pytest.mark.unit
async def test_create_queue(async_db_fixture, queue_args):
    queue_id = await async_db_fixture.create_queue(**queue_args)
    assert queue_id is not None

    # Verify queue was created
    queue = await async_db_fixture._queues.find_one({"_id": queue_id})
...
async def test_create_task(
    async_db_fixture, queue_args, get_task_args, get_full_task_args
):
    # Create queue first
    queue_id = await async_db_fixture.create_queue(**queue_args)

    # Test 1. Create task with minimal args
    task_id = await async_db_fixture.create_task(**get_task_args(queue_id))
    assert task_id is not None

    # Verify task was created
    task = await async_db_fixture._tasks.find_one({"_id": task_id})
    assert task is not None
    assert task["queue_id"] == queue_id
    assert task["status"] == TaskState.PENDING

    # Test 2. Create task with all args
    task_id = await async_db_fixture.create_task(**get_full_task_args(queue_id))
    assert task_id is not None
...
@pytest.mark.integration
@pytest.mark.unit
async def test_fetch_task(async_db_fixture, queue_args, get_task_args):
    # Setup
    queue_id = await async_db_fixture.create_queue(**queue_args)

    # 1. Basic fetch
    await async_db_fixture.create_task(**get_task_args(queue_id))

    # Fetch task
    task = await async_db_fixture.fetch_task(queue_id=queue_id)
...
@pytest.mark.integration
@pytest.mark.unit
async def test_create_duplicate_queue(async_db_fixture, queue_args):
    """Test creating a queue with duplicate name."""
    # Create first queue
    await async_db_fixture.create_queue(**queue_args)
...
@pytest.mark.integration
@pytest.mark.unit
async def test_create_queue_invalid_name(async_db_fixture):
    """Test creating a queue with invalid name."""
    with pytest.raises(HTTPException) as exc:
        await async_db_fixture.create_queue(queue_name="", password="test")
...
@pytest.mark.integration
@pytest.mark.unit
async def test_create_task_invalid_args(async_db_fixture, queue_args):
    ...
    with pytest.raises(HTTPException) as exc:
        await async_db_fixture.create_task(**task_data)
    assert exc.value.status_code == 400


@pytest.mark.integration
@pytest.mark.unit
async def test_heartbeat_timeout(async_db_fixture, queue_args, get_task_args):
    """Test task execution timeout using freezegun."""
    # Create queue and task with a heartbeat timeout
    queue_id = await async_db_fixture.create_queue(**queue_args)
    task_id = await async_db_fixture.create_task(
        **get_task_args(
            queue_id,
            override_fields={
    ...
    with freeze_time("2025-01-01 12:00:00") as frozen_time:
        ...
        task = await async_db_fixture.fetch_task(
            queue_id=queue_id,
        )
        ...
        transitioned = await async_db_fixture.handle_timeouts()
...
@pytest.mark.integration
@pytest.mark.unit
async def test_task_retry_on_timeout(async_db_fixture, queue_args, get_task_args):
    ...
    with freeze_time("2025-01-01 12:00:00") as frozen_time:
        ...
        await async_db_fixture.handle_timeouts()
...
@pytest.mark.integration
@pytest.mark.unit
async def test_update_task_status(async_db_fixture, queue_args, get_task_args):
    ...
    assert task["status"] == TaskState.RUNNING
    assert task["_id"] == task_id
    assert await async_db_fixture.update_task_status(
    ...
    task_id = await async_db_fixture.create_task(
        **get_task_args(queue_id)
    )  # Create new task
    task = await async_db_fixture.fetch_task(queue_id=queue_id)
    assert task["_id"] == task_id
    assert await async_db_fixture.update_task_status(queue_id, task_id, "failed")
    task = await async_db_fixture._tasks.find_one({"_id": task_id})
    ...
    task_id = await async_db_fixture.create_task(**get_task_args(queue_id))
    assert await async_db_fixture.update_task_status(queue_id, task_id, "cancelled")
    task = await async_db_fixture._tasks.find_one({"_id": task_id})
    ...
    with pytest.raises(HTTPException) as exc:
        await async_db_fixture.update_task_status(queue_id, task_id, "invalid_status")
    assert exc.value.status_code == 400
    ...
    # with pytest.raises(HTTPException) as exc:
    #     await async_db_fixture.update_task_status("non_existent_queue", task_id, "success")
    # assert exc.value.status_code == 404
    # assert "Queue 'non_existent_queue' not found" in exc.value.detail

    # Test case 8: Non-existent task
    with pytest.raises(HTTPException) as exc:
        await async_db_fixture.update_task_status(
            queue_id, "non_existent_task", "success"
        )
    assert exc.value.status_code == 404
...
@pytest.mark.integration
@pytest.mark.unit
async def test_task_fsm_consistency(async_db_fixture, queue_args, get_task_args):
    ...
    event_mapping = {
        "fetch": (
            TaskState.PENDING,
            lambda queue_id, task_id: async_db_fixture.fetch_task(queue_id=queue_id),
            TaskFSM.fetch,
        ),
        "report_success": (
            TaskState.RUNNING,
            partial(async_db_fixture.update_task_status, report_status="success"),
            TaskFSM.complete,
        ),
        "report_failed": (
            TaskState.RUNNING,
            partial(async_db_fixture.update_task_status, report_status="failed"),
            TaskFSM.fail,
        ),
        "report_pending_cancelled": (
            TaskState.PENDING,
            partial(async_db_fixture.update_task_status, report_status="cancelled"),
            TaskFSM.cancel,
        ),
        ...
        "reset_pending": (
            TaskState.PENDING,
            async_db_fixture.update_task_and_reset_pending,
            TaskFSM.reset,
        ),
        "reset_running": (
            TaskState.RUNNING,
    ...
    async def clear_tasks():
        await async_db_fixture._tasks.delete_many({})

    async def get_pending():
        task_id = await async_db_fixture.create_task(**get_task_args(queue_id))
        task = await async_db_fixture._tasks.find_one({"_id": task_id})
        assert task["status"] == TaskState.PENDING
        return task, async_db_fixture

    async def get_running():
        task, async_db_fixture = await get_pending()
        task = await async_db_fixture.fetch_task(queue_id=queue_id)
        assert task["status"] == TaskState.RUNNING
        return task, async_db_fixture
    ...
    async def get_failed():
        task_id = await async_db_fixture.create_task(**get_task_args(queue_id))
        task = await async_db_fixture._tasks.find_one_and_update(
            {"_id": task_id},
    ...
    for event_name, (init_state, db_func, fsm_func) in event_mapping.items():
        # Fetch task
        task, async_db_fixture = get_initial_state_func[init_state]()
        task_id = task["_id"]
...
@pytest.mark.integration
@pytest.mark.unit
async def test_worker_crash_no_dispatch(async_db_fixture, queue_args, get_task_args):
    """Test that crashed workers don't receive new tasks."""
    # Setup
    queue_id = await async_db_fixture.create_queue(**queue_args)

    # Create worker
    worker_id = await async_db_fixture.create_worker(
        queue_id=queue_id,
        max_retries=3,
    )
    ...
    for i in range(3):
        task_ids.append(
            await async_db_fixture.create_task(
    ...
    for _ in range(3):  # Worker max_retries is 3 by default
        ...
        worker = await async_db_fixture._workers.find_one({"_id": worker_id})
        assert worker["status"] == WorkerState.ACTIVE

        # Fetch task
        task = await async_db_fixture.fetch_task(queue_id=queue_id, worker_id=worker_id)
        assert task is not None
    ...
    with pytest.raises(HTTPException) as exc:
        await async_db_fixture.fetch_task(queue_id=queue_id, worker_id=worker_id)
    assert exc.value.status_code == 400
    assert "crashed" in exc.value.detail

    # Re-activate worker
    await async_db_fixture.update_worker_status(
...
@pytest.mark.integration
@pytest.mark.unit
async def test_worker_suspended_no_dispatch(
    async_db_fixture, queue_args, get_task_args
):
    ...
    # Create worker
    worker_id = await async_db_fixture.create_worker(queue_id=queue_id)
...
@pytest.mark.integration
@pytest.mark.unit
async def test_fetch_priority(async_db_fixture, queue_args):
    ...
    # Create tasks
    await async_db_fixture.create_task(**task_args_high)
    await async_db_fixture.create_task(**task_args_medium_1)
    await async_db_fixture.create_task(**task_args_medium_2)
    await async_db_fixture.create_task(**task_args_low)
...
@pytest.mark.integration
@pytest.mark.unit
async def test_fetch_extra_filter(async_db_fixture, queue_args):
    # Setup: Create a queue
    queue_id = await async_db_fixture.create_queue(**queue_args)
    ...
    # Create tasks
    await async_db_fixture.create_task(**task_args_1)
    await async_db_fixture.create_task(**task_args_2)
    await async_db_fixture.create_task(**task_args_3)

    # Test 1. query by self-defined tag
    extra_filter = {"metadata": {"tag": "b"}}

    task = await async_db_fixture.fetch_task(
        queue_id=queue_id, extra_filter=extra_filter
    )
...
class TestTaskRequiredFieldFetching:
    ...
    # @pytest.mark.asyncio
    async def test_fetch_leaf_match(self, async_db_fixture, queue_args):
        ...
        for args in task_args:
            await async_db_fixture.create_task(**args)
    ...
    # @pytest.mark.asyncio
    async def test_fetch_non_leaf_match(self, async_db_fixture, queue_args):
    ...
    # @pytest.mark.asyncio
    async def test_fetch_no_match(self, async_db_fixture, queue_args):
    ...
    # @pytest.mark.asyncio
    async def test_fetch_with_multiple_matches(self, async_db_fixture, queue_args):
