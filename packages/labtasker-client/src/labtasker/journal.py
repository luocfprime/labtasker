...
class LocalRunJournal:
    ...
    def create(
        ...
        *,
        ...
        root = (Path.cwd() / ".labtasker" if labtasker_root is None else labtasker_root).resolve()
...
def _validate_record(value: object) -> RunRecord:
    ...
    if not isinstance(value, dict) or set(value) != fields:
        raise ValueError(f"run.json does not match journal schema version {JOURNAL_SCHEMA_VERSION}")
...
def _validate_endpoint(value: object) -> EndpointRecord:
    ...
    managed_local = value["managed_local"]
    if not isinstance(managed_local, bool):
        raise ValueError("run.json endpoint managed_local is invalid")
    if connection == "http":
        if managed_local:
            raise ValueError("run.json HTTP endpoint cannot be managed local")
        if not isinstance(value["url"], str) or not value["url"]:
            raise ValueError("run.json HTTP endpoint URL is invalid")
        if any(value[field] is not None for field in ("socket", "labtasker_root", "database")):
            raise ValueError("run.json HTTP endpoint contains local paths")
    elif connection == "socket":
        if value["url"] is not None or not isinstance(value["socket"], str):
            raise ValueError("run.json socket endpoint is invalid")
        managed_paths = ("labtasker_root", "database")
        if managed_local and any(
            not isinstance(value[field], str) or not value[field] for field in managed_paths
        ):
            raise ValueError("run.json managed-local endpoint paths are invalid")
        if not managed_local and any(value[field] is not None for field in managed_paths):
            raise ValueError("run.json external socket contains managed-local paths")
    else:
        raise ValueError("run.json endpoint connection is invalid")
